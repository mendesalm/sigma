...
router = APIRouter(prefix="/check-in", tags=["Check-in"], responses={404: {"description": "Não encontrado"}})
...
def qr_code_check_in(check_in_data: attendance_schema.QrCheckInRequest, db: Session = Depends(get_db)):
    ...
    return attendance_service.record_qr_code_attendance(db=db, check_in_data=check_in_data)
