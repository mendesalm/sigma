...
def require_lodge_admin(current_user: dict):
    ...
    if user_type != "super_admin" and user_type != "webmaster":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Acesso negado às configurações financeiras.")
...
@router.get(
    ...
    responses={200: {"description": "Configurações retornadas com sucesso"}, 403: {"description": "Acesso negado"}},
)
...
def get_categories(
    type: schemas.TransactionType | None = Query(None, description="Filtrar categorias por Receita ou Despesa"),
    db: Session = Depends(get_db),
    current_user: dict = Depends(get_current_user_payload),
):
...
def list_financial_transactions(
    ...
    transaction_type: schemas.TransactionTypeEnum | None = Query(
