...
from app.modules.communication.schemas.publication_schemas import PublicationCreate, PublicationResponse
from app.modules.communication.services.publication_service import PublicationService
from database import get_db
from dependencies import UserContext, get_current_active_user_with_permissions
from models.models import Member, MemberLodgeAssociation, PublicationTypeEnum
...
def check_upload_permission(context: UserContext, lodge_id: int):
    ...
    if context.user_type == "member":
        ...
        current_roles = [rh for rh in user.role_history if rh.lodge_id == lodge_id and rh.end_date is None]
...
def check_read_permission(context: UserContext, lodge_id: int, db: Session):
    ...
    if context.user_type == "member":
        # Check association
        is_member = (
            db.query(MemberLodgeAssociation)
            .filter(
...
def create_publication(
    ...
    pub_data = PublicationCreate(title=title, type=type, content=content, valid_until=valid_until)
    ...
    return PublicationService.create_publication(
        db=db, publication_data=pub_data, file=file, author_id=author_id, lodge_id=lodge_id
    )
