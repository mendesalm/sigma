...
from app.modules.communication.services import event_service
...
def list_calendars(db: Session = Depends(get_db), current_user_payload: dict = Depends(get_current_user_payload)):
...
def get_calendar(
    calendar_id: int, db: Session = Depends(get_db), current_user_payload: dict = Depends(get_current_user_payload)
):
...
def update_existing_calendar(
    ...
    return event_service.update_calendar(
        db=db, calendar_id=calendar_id, calendar_update=calendar_update, current_user_payload=current_user_payload
    )
...
def list_events(
    ...
    return event_service.get_events_by_lodge(
        db=db, current_user_payload=current_user_payload, start_date=start_date, end_date=end_date
    )
...
def get_event(
    event_id: int, db: Session = Depends(get_db), current_user_payload: dict = Depends(get_current_user_payload)
):
...
def update_existing_event(
    ...
    return event_service.update_event(
        db=db, event_id=event_id, event_update=event_update, current_user_payload=current_user_payload
    )
