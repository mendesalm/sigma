...
from app.modules.communication.schemas import classified_schema
...
def list_classifieds(db: Session = Depends(get_db), current_user_payload: dict = Depends(get_current_user_payload)):
    return classified_service.get_all_active_classifieds(db)


@router.get("/my", response_model=list[classified_schema.ClassifiedOut])
def list_my_classifieds(db: Session = Depends(get_db), current_user_payload: dict = Depends(get_current_user_payload)):
...
def reactivate_classified(
    classified_id: int, db: Session = Depends(get_db), current_user_payload: dict = Depends(get_current_user_payload)
):
