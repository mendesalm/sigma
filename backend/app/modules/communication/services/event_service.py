...
from app.modules.communication.schemas import calendar_schema, event_schema
...
def create_calendar(
    db: Session, calendar_data: calendar_schema.CalendarCreate, current_user_payload: dict
) -> models.Calendar:
    """
...
def get_calendar_by_id(db: Session, calendar_id: int, current_user_payload: dict) -> models.Calendar:
    """
    ...
    calendar = (
        ...
        .filter(models.Calendar.id == calendar_id, models.Calendar.lodge_id == lodge_id)
        .first()
    )
...
def get_calendars_by_lodge(db: Session, current_user_payload: dict) -> list[models.Calendar]:
    """
...
def update_calendar(
    db: Session, calendar_id: int, calendar_update: calendar_schema.CalendarUpdate, current_user_payload: dict
) -> models.Calendar:
    """
...
def delete_calendar(db: Session, calendar_id: int, current_user_payload: dict) -> models.Calendar:
    """
...
def create_event(db: Session, event_data: event_schema.EventCreate, current_user_payload: dict) -> models.Event:
    """
...
def get_event_by_id(db: Session, event_id: int, current_user_payload: dict) -> models.Event:
    """
    Busca um evento pelo ID, garantindo que pertença à loja do usuário.
    """
    lodge_id = current_user_payload.get("lodge_id")
    event = db.query(models.Event).filter(models.Event.id == event_id, models.Event.lodge_id == lodge_id).first()
...
def get_events_by_lodge(
    db: Session, current_user_payload: dict, start_date: datetime | None = None, end_date: datetime | None = None
) -> list[models.Event]:
    """
...
def update_event(
    db: Session, event_id: int, event_update: event_schema.EventUpdate, current_user_payload: dict
) -> models.Event:
    """
...
def delete_event(db: Session, event_id: int, current_user_payload: dict) -> models.Event:
    """
