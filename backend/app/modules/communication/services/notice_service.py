...
from app.modules.communication.schemas.notice_schemas import NoticeCreate, NoticeUpdate
...
class NoticeService:
    ...
    def get_notices(db: Session, lodge_id: int, active_only: bool = True) -> list[Notice]:
        ...
        if active_only:
            ...
            today = date.today()
            query = query.filter(
                Notice.is_active, (Notice.expiration_date.is_(None)) | (Notice.expiration_date >= today)
            )
        return query.all()
    ...
    @staticmethod
    def update_notice(db: Session, notice_id: int, update_data: NoticeUpdate) -> Notice | None:
