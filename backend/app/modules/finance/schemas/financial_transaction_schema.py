...
class LodgeFinancialConfigBase(BaseModel):
    active_gateway: GatewayProvider = Field(
        GatewayProvider.NONE, description="Provedor de pagamento ativo (ex: ASAAS, IUGU)"
    )
    strict_budget_control: bool = Field(False, description="Se verdadeiro, bloqueia despesas não orçadas")
    recalculate_late_fees: bool = Field(
        True, description="Se verdadeiro, recalcula juros e multas ao atualizar transações vencidas"
    )
...
class LodgeFinancialConfigCreate(LodgeFinancialConfigBase):
    gateway_api_key: str | None = Field(None, description="Chave de API do gateway de pagamento")
    gateway_webhook_secret: str | None = Field(None, description="Token/Secret para validação de webhooks")
...
class LodgeFinancialConfigResponse(LodgeFinancialConfigBase):
    ...
    has_api_key: bool = Field(
        ..., description="Indica se existe uma chave de API configurada (mascarada por segurança)"
    )
...
class FinancialCategoryCreate(FinancialCategoryBase):
    class Config:
        json_schema_extra = {"example": {"name": "Mensalidade", "type": "RECEITA", "is_active": True}}
...
class AnnualBudgetCreate(AnnualBudgetBase):
    ...
    class Config:
        json_schema_extra = {"example": {"year": 2026, "month": 2, "budgeted_amount": 5000.0, "category_id": 1}}
...
class FinancialTransactionBase(BaseModel):
    ...
    member_id: int | None = Field(None, description="ID do membro associado (se for cobrança/pagamento de membro)")
    category_id: int | None = Field(None, description="ID da categoria financeira (Plano de contas)")
...
class FinancialTransactionUpdate(BaseModel):
    ...
    due_date: date | None = Field(None, description="Nova data de vencimento")
    competency_date: date | None = Field(None, description="Nova data de competência")
    status: TransactionStatus | None = Field(None, description="Novo status da transação")

    interest_amount: float | None = Field(None, description="Atualizar valor de juros")
    penalty_amount: float | None = Field(None, description="Atualizar valor de multa")
    discount_amount: float | None = Field(None, description="Atualizar valor de desconto")
    paid_amount: float | None = Field(None, description="Atualizar valor pago final")
    payment_date: date | None = Field(None, description="Atualizar data efetiva de pagamento")

    class Config:
        json_schema_extra = {"example": {"status": "PAGO", "paid_amount": 150.0, "payment_date": "2026-02-10"}}
...
class FinancialTransactionResponse(FinancialTransactionBase):
    ...
    paid_amount: float | None = Field(None, description="Valor total pago")
    payment_date: date | None = Field(None, description="Data efetiva do pagamento")

    gateway_id: str | None = Field(None, description="ID da cobrança gerado pelo gateway externo")
    gateway_link: str | None = Field(None, description="URL do boleto ou fatura externa")
    gateway_barcode: str | None = Field(None, description="Linha digitável do boleto")
    gateway_pix_qrcode: str | None = Field(None, description="Chave PIX ou Payload do QRCode")
    gateway_status: str | None = Field(None, description="Status bruto retornado pelo webhook do gateway")
