...
from app.modules.core.services.email_service import send_library_late_notice_email, send_library_waitlist_notice_email
from app.modules.library.schemas.loan_schema import LoanCreate
...
class WaitlistService:
    ...
    def create_waitlist(db: Session, lodge_id: int, member_id: int, waitlist_in: WaitlistCreate) -> Waitlist:
        ...
        new_waitlist = Waitlist(
            book_id=waitlist_in.book_id, lodge_id=lodge_id, member_id=member_id, status=WaitlistStatusEnum.WAITING
        )
        db.add(new_waitlist)
        db.commit()
    ...
    @staticmethod
    def notify_next_in_line(db: Session, book_id: int, lodge_id: int) -> Waitlist | None:
        ...
        next_in_line = (
            ...
            .order_by(Waitlist.request_date.asc())
            .first()
        )
        ...
        if next_in_line:
            ...
            if next_in_line.member and next_in_line.member.email:
                send_library_waitlist_notice_email(to=next_in_line.member.email, book_title=next_in_line.book.title)
    ...
    def expire_notified_waitlists(db: Session):
        ...
        expired = (
            db.query(Waitlist)
            .filter(and_(Waitlist.status == WaitlistStatusEnum.NOTIFIED, Waitlist.expiration_date < now))
            .all()
        )
...
class LoanService:
    ...
    def create_loan(db: Session, lodge_id: int, loan_in: LoanCreate) -> Loan:
        ...
        active_loans = (
            db.query(Loan)
            .filter(and_(Loan.member_id == loan_in.member_id, Loan.status == LoanStatusEnum.ACTIVE))
            .count()
        )
        ...
        if member_degree_level < book_degree_level:
            raise ValueError(
                f"O seu grau maçônico não permite o empréstimo deste livro. Grau exigido: {book_degree_level}"
            )
    ...
    def return_loan(db: Session, loan_id: int, lodge_id: int) -> Loan:
        loan = (
            db.query(Loan).join(LibraryItem).filter(and_(Loan.id == loan_id, LibraryItem.lodge_id == lodge_id)).first()
        )
    ...
    def get_lodge_active_loans(db: Session, lodge_id: int, skip: int = 0, limit: int = 100) -> list[Loan]:
        return (
            ...
            .join(LibraryItem)
            .filter(
                and_(LibraryItem.lodge_id == lodge_id, Loan.status.in_([LoanStatusEnum.ACTIVE, LoanStatusEnum.LATE]))
            )
    ...
    def check_late_loans(db: Session):
        now = datetime.now()
        late_loans = db.query(Loan).filter(and_(Loan.status == LoanStatusEnum.ACTIVE, Loan.due_date < now)).all()

        for loan in late_loans:
            loan.status = LoanStatusEnum.LATE
            if loan.member and loan.member.email:
                send_library_late_notice_email(to=loan.member.email, book_title=loan.item.book.title)
