...
from app.modules.library.schemas.book_schema import BookCreate, BookUpdate
from models.models import Book


class BookService:
    @staticmethod
    async def fetch_book_from_isbn(isbn: str) -> dict | None:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("totalItems", 0) > 0:
                        ...
                        return {
                            ...
                            "publish_year": int(volume_info.get("publishedDate", "0")[:4])
    ...
    @staticmethod
    def list_books(db: Session, skip: int = 0, limit: int = 100, search: str | None = None) -> list[Book]:
        query = db.query(Book)
        if search:
            search_filter = f"%{search}%"
            query = query.filter(
                or_(Book.title.ilike(search_filter), Book.author.ilike(search_filter), Book.isbn.ilike(search_filter))
            )
