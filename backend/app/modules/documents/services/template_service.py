...
from app.modules.documents.schemas import template_schema
...
def sync_document_settings_to_local_templates(db: Session, lodge_id: int, document_settings: dict) -> None:
    ...
    for doc_type, settings in document_settings.items():
        ...
        local_tpl = (
            ...
            .filter(LocalDocumentTemplate.lodge_id == lodge_id, LocalDocumentTemplate.document_type == doc_type)
            .first()
        )

        if not local_tpl:
            local_tpl = LocalDocumentTemplate(lodge_id=lodge_id, document_type=doc_type, is_active=True)
            db.add(local_tpl)
