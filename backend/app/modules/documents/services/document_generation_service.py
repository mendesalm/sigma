...
from app.modules.documents.services.document_strategies.balaustre_strategy import BalaustreStrategy
from app.modules.documents.services.document_strategies.certificate_strategy import CertificateStrategy
from app.modules.documents.services.document_strategies.congratulation_strategy import CongratulationStrategy
from app.modules.documents.services.document_strategies.electoral_balaustre_strategy import ElectoralBalaustreStrategy
from app.modules.documents.services.document_strategies.invitation_strategy import InvitationStrategy
from app.modules.documents.services.document_strategies.prancha_strategy import PranchaStrategy
from app.modules.documents.services.pdf_service import PdfService
...
from app.modules.documents.services import template_service
...
class DocumentGenerationService:
    def __init__(self, db_session: Session | None = None):
        ...
        # Caminhos base
        backend_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../.."))  # .../sigma/backend

        # Configura o Loader para buscar em múltiplos locais
        template_paths = [
            os.path.join(backend_dir, "app", "shared", "templates"),  # Legacy/Fallback
            os.path.join(backend_dir, "app", "shared", "templates", "partials"),  # Legacy Partials (header_master.html)
    ...
    async def _generate_pdf_from_html(self, html_content: str) -> bytes:
        """Wrapper para gerar PDF a partir de HTML usando o PdfService."""
        return await self.pdf_service.generate_pdf_from_html(html_content)
    ...
    def _get_base64_asset(self, asset_path: str) -> str:
        ...
        base_path = os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../..")), "app", "shared", "assets")
    ...
    def _get_lodge_logo(self, lodge_id: int) -> str:
        ...
        base_storage = os.path.join(os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../..")), "storage", "lodges")
        logo_path = None
    ...
    def _get_merged_document_settings(self, db: Session, doc_type: str, lodge_id: int | None) -> dict:
        ...
        """
        from app.modules.documents.schemas.document_settings_schema import DEFAULT_ELEMENTS_MAP
        ...
        global_elements = (
            (global_tpl.structural_elements_json or DEFAULT_ELEMENTS_MAP.get(doc_type, []))
            if global_tpl
            else DEFAULT_ELEMENTS_MAP.get(doc_type, [])
        )
    ...
    def _compose_document_from_elements(
        ...
        sorted_elements = sorted(
            structural_elements, key=lambda el: el.get("order", 0) if isinstance(el, dict) else el.order
        )
        ...
        for el in sorted_elements:
            ...
            # Remove legacy page-border div and style if it is hardcoded inside the text
            if override and isinstance(override, str):
                import re

                override = re.sub(r'<div class="page-border">\s*</div>', "", override)
                override = re.sub(
                    r"<style>.*?\.page-border\s*{[^}]*}.*?</style>", "", override, flags=re.IGNORECASE | re.DOTALL
                )
    ...
    def get_default_templates(self, doc_type: str) -> dict:
        ...
        defaults_dir = os.path.join(backend_dir, "app", "shared", "templates", "defaults")
    ...
    async def generate_preview_html(
        self, doc_type: str, settings: dict, lodge_id: int = None, session_id: int = None
    ) -> str:
        """
        ...
        try:
            ...
            # Extração de styles dinâmicos para page_settings do preview
            # Na UI de preview, o frontend envia `currentSettings` puro, que não tem o objeto `page_settings`
            # populado pelo método de `buildPayloadV2`. Nós precisamos mapeá-lo on-the-fly para o base_paper.html.
            raw_styles = settings.get("styles", {})
            page_settings = settings.get("page_settings", {})
            if not page_settings and isinstance(raw_styles, dict) and raw_styles:
                page_settings = {
                    "format": raw_styles.get("page_size", "A4"),
                    "orientation": raw_styles.get("orientation", "portrait"),
                    "margin_top": raw_styles.get("page_margin", "1cm"),
                    "margin_bottom": raw_styles.get("page_margin", "1cm"),
                    "margin_left": raw_styles.get("page_margin", "1cm"),
                    "margin_right": raw_styles.get("page_margin", "1cm"),
                    "background_color": raw_styles.get("background_color", "#ffffff"),
                    "background_image": raw_styles.get("background_image"),
                    "show_border": raw_styles.get("show_border", False),
                    "border_style": raw_styles.get("border_style", "solid"),
                    "border_color": raw_styles.get("border_color", "#000000"),
                    "border_width": raw_styles.get("border_width", "1px"),
                    "page_padding": raw_styles.get("page_padding", "0cm"),
                }
