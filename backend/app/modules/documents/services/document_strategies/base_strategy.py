...
from app.modules.documents.schemas.document_settings_schema import DocumentSettings
...
class DocumentStrategy(ABC):
    ...
    def _apply_dynamic_styles(self, context: dict[str, Any], styles_payload: Any):
        ...
        try:
            from app.modules.documents.schemas.document_settings_schema import DocumentStyles
            ...
            # Ensure Jinja receives a dictionary so .get() methods on styles work inside templates
            if hasattr(styles_model, "model_dump"):
                context["styles"] = styles_model.model_dump()
            elif hasattr(styles_model, "dict"):
                context["styles"] = styles_model.dict()
            elif isinstance(styles_model, dict):
                context["styles"] = styles_model
            else:
                context["styles"] = dict(styles_model)
            ...
            .page-border {{
                ...
                z-index: 2147483647;
                pointer-events: none;
                {"border: none !important; border-image: none !important;" if not styles_model.show_border else ""}
            }}
