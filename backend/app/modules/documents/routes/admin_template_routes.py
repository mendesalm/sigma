...
@router.post("/upload_asset", summary="Upload de Asset para Modelo Universal")
async def upload_universal_asset(
    file: UploadFile = File(...),
    db: Session = Depends(database.get_db),
    current_user: dict = Depends(dependencies.get_current_super_admin),
):
    """
    Submete uma imagem ou asset global utilizado em templates do sistema.
    """
    safe_filename = file.filename.replace(" ", "_").lower()
    storage_dir = os.path.join("storage", "lodges", "model", "assets")
    ...
    url_path = f"/storage/lodges/model/assets/{safe_filename}"
    return {"url": url_path}
