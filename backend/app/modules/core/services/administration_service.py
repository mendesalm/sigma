...
from app.modules.core.schemas import administration_schema
...
def get_current_administration(db: Session, lodge_id: int) -> models.Administration | None:
    ...
    return (
        db.query(models.Administration)
        .filter(models.Administration.lodge_id == lodge_id, models.Administration.is_current)
        .options(
            joinedload(models.Administration.role_histories).joinedload(models.RoleHistory.role),
            joinedload(models.Administration.role_histories).joinedload(models.RoleHistory.member),
        )
...
def create_administration(
    db: Session, data: administration_schema.AdministrationCreate, current_user_payload: dict
...
def update_administration(
    db: Session, admin_id: int, data: administration_schema.AdministrationUpdate, current_user_payload: dict
) -> models.Administration:
    """
    ...
    db_admin = (
        db.query(models.Administration)
        .filter(models.Administration.id == admin_id, models.Administration.lodge_id == lodge_id)
        .first()
    )
...
def _unset_current_administration(db: Session, lodge_id: int, exclude_id: int = None):
    query = db.query(models.Administration).filter(
        models.Administration.lodge_id == lodge_id, models.Administration.is_current
    )
    if exclude_id:
