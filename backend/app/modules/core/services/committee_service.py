...
from app.modules.core.schemas import committee_schema
...
def get_committees(db: Session, lodge_id: int) -> list[models.Committee]:
    return (
        ...
        .filter(models.Committee.lodge_id == lodge_id)
        .options(
...
def create_committee(db: Session, committee: committee_schema.CommitteeCreate, lodge_id: int) -> models.Committee:
    ...
    for member_id in committee.member_ids:
        if member_id == committee.president_id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="O presidente não pode ser listado como membro."
            )
    ...
    # 4. Add Members
    for member_id in committee.member_ids:
        db_member = models.CommitteeMember(committee_id=db_committee.id, member_id=member_id, role="Membro")
        db.add(db_member)
...
def update_committee(
    db: Session, committee_id: int, committee_update: committee_schema.CommitteeUpdate, lodge_id: int
) -> models.Committee:
    db_committee = (
        db.query(models.Committee)
        .filter(models.Committee.id == committee_id, models.Committee.lodge_id == lodge_id)
        .first()
    )
    if not db_committee:
    ...
    # Update basic fields
    if committee_update.name:
        db_committee.name = committee_update.name
    if committee_update.description:
        db_committee.description = committee_update.description
    if committee_update.committee_type:
        db_committee.committee_type = committee_update.committee_type
    if committee_update.start_date:
        db_committee.start_date = committee_update.start_date
    if committee_update.end_date:
        db_committee.end_date = committee_update.end_date
    ...
    if committee_update.member_ids is not None:
        ...
        for member_id in committee_update.member_ids:
            ...
            db_member = models.CommitteeMember(committee_id=committee_id, member_id=member_id, role="Membro")
            db.add(db_member)
