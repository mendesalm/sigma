...
@router.get("/stats")
def get_dashboard_stats(db: Session = Depends(get_db), payload: dict = Depends(get_current_user_payload)):
    ...
    next_events = (
        ...
        .filter(models.Event.lodge_id == lodge_id, models.Event.start_time >= datetime.now())
    ...
    active_member_dates = (
        ...
        .options(joinedload(models.Member.masonic_history))
        .all()
    )
    ...
    for m in active_member_dates:
        ...
        initiation_event = next((ev for ev in m.masonic_history if ev.event_type and ev.event_type.name == "INITIATION"), None)
        if initiation_event and initiation_event.session_date:
            initiation = get_next_occurrence(initiation_event.session_date, today)
        ...
        elevation_event = next((ev for ev in m.masonic_history if ev.event_type and ev.event_type.name == "ELEVATION"), None)
        if elevation_event and elevation_event.session_date:
            elevation = get_next_occurrence(elevation_event.session_date, today)
            if elevation and today <= elevation <= limit_date:
                upcoming_birthdays.append({"name": f"Ir. {m.full_name}", "date": elevation, "type": "elevacao"})

        exaltation_event = next((ev for ev in m.masonic_history if ev.event_type and ev.event_type.name == "EXALTATION"), None)
        if exaltation_event and exaltation_event.session_date:
            exaltation = get_next_occurrence(exaltation_event.session_date, today)
    ...
    active_notices_query = (
        ...
        .filter(
            ...
            or_(models.Notice.expiration_date is None, models.Notice.expiration_date >= today),
        )
    ...
    next_session = (
        ...
        .order_by(models.MasonicSession.session_date)
        .first()
    )
    ...
    classifieds_count = (
        ...
        .filter(models.Classified.lodge_id == lodge_id, models.Classified.status == "Ativo")
        .count()
    )

    # 7. Dining Scale (Next ones)
    dining_scale_records = (
        db.query(models.DiningScale)
        .join(models.Member)
        .filter(models.DiningScale.lodge_id == lodge_id, models.DiningScale.date >= today)
        .order_by(models.DiningScale.date)
        .limit(5)
        .all()
    )
    ...
    if lodge:
        ...
        if lodge.street_address:
            address_parts.append(lodge.street_address)
        if lodge.street_number:
            address_parts.append(lodge.street_number)
        if lodge.neighborhood:
            address_parts.append(lodge.neighborhood)
        if lodge.city:
            address_parts.append(lodge.city)
        if lodge.state:
            address_parts.append(lodge.state)
        if lodge.zip_code:
            address_parts.append(f"CEP {lodge.zip_code}")
...
def get_calendar_events(
    month: int, year: int, db: Session = Depends(get_db), payload: dict = Depends(get_current_user_payload)
):
    lodge_id = payload.get("lodge_id")
    ...
    sessions = (
        db.query(models.MasonicSession)
        .filter(
            models.MasonicSession.lodge_id == lodge_id, models.MasonicSession.session_date.between(start_date, end_date)
        )
        .all()
    )
    ...
    events = (
        db.query(models.Event)
        .filter(models.Event.lodge_id == lodge_id, func.date(models.Event.start_time).between(start_date, end_date))
        .all()
    )
    ...
    for s in sessions:
        calendar_events.append(
            {"date": s.session_date.day, "title": s.title, "type": "sessao", "full_date": s.session_date}
        )

    # Map Events
    for e in events:
        calendar_events.append(
            {"date": e.start_time.day, "title": e.title, "type": "evento", "full_date": e.start_time.date()}
        )
    ...
    for m in active_member_dates:
        ...
        initiation_event = next((ev for ev in m.masonic_history if ev.event_type and ev.event_type.name == "INITIATION"), None)
        if initiation_event and initiation_event.session_date and initiation_event.session_date.month == month:
            calendar_events.append(
                {
                    "date": initiation_event.session_date.day,
                    "title": f"Ir. {m.full_name}",
                    "type": "iniciacao",
                    "full_date": date(year, month, initiation_event.session_date.day),
                }
            )

        elevation_event = next((ev for ev in m.masonic_history if ev.event_type and ev.event_type.name == "ELEVATION"), None)
        if elevation_event and elevation_event.session_date and elevation_event.session_date.month == month:
            calendar_events.append(
                {
                    "date": elevation_event.session_date.day,
                    "title": f"Ir. {m.full_name}",
                    "type": "elevacao",
                    "full_date": date(year, month, elevation_event.session_date.day),
                }
            )

        exaltation_event = next((ev for ev in m.masonic_history if ev.event_type and ev.event_type.name == "EXALTATION"), None)
        if exaltation_event and exaltation_event.session_date and exaltation_event.session_date.month == month:
            calendar_events.append(
                {
                    "date": exaltation_event.session_date.day,
                    "title": f"Ir. {m.full_name}",
                    "type": "exaltacao",
                    "full_date": date(year, month, exaltation_event.session_date.day),
                }
            )
