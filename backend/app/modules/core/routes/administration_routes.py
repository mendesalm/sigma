...
from app.modules.core.services import administration_service
...
def get_current_administration(
    db: Session = Depends(get_db), current_user_payload: dict = Depends(dependencies.get_current_user_payload)
):
    lodge_id = current_user_payload.get("lodge_id")
    if not lodge_id:
        return None
