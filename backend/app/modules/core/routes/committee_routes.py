...
from app.modules.core.services import committee_service
...
def read_committees(db: Session = Depends(get_db), current_user_payload: dict = Depends(get_current_user_payload)):
...
def delete_committee(
    committee_id: int, db: Session = Depends(get_db), current_user_payload: dict = Depends(get_current_user_payload)
):
    lodge_id = current_user_payload.get("lodge_id")
