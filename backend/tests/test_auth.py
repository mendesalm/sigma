...
def test_login_failure(client, sample_super_admin):
    ...
    response = client.post("/auth/login", data={"username": "admin@test.com", "password": "WrongPassword"})
...
def test_access_protected_route_with_token(client, super_admin_token):
    ...
    response = client.get("/super-admins/", headers={"Authorization": f"Bearer {super_admin_token}"})
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
