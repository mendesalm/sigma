def test_create_and_get_book(client, webmaster_token):
    ...
    response = client.post("/library/books", json=book_data, headers={"Authorization": f"Bearer {webmaster_token}"})
    ...
    # 2. List Books
    response = client.get("/library/books", headers={"Authorization": f"Bearer {webmaster_token}"})
    ...
    item_data = {"book_id": book_id, "acquisition_date": "2023-01-01", "condition": "Novo", "notes": "Doação"}

    response = client.post("/library/items", json=item_data, headers={"Authorization": f"Bearer {webmaster_token}"})
    assert response.status_code == 201
    ...
    response = client.get("/library/items", headers={"Authorization": f"Bearer {webmaster_token}"})
    assert response.status_code == 200
