...
def test_create_and_get_notice(client, webmaster_token):
    ...
    response = client.post("/notices/", json=notice_data, headers={"Authorization": f"Bearer {webmaster_token}"})
    ...
    # 2. Get Notices
    response = client.get("/notices/?lodge_id=1", headers={"Authorization": f"Bearer {webmaster_token}"})
    assert response.status_code == 200
    ...
    # 3. Update Notice
    update_data = {"title": "Aviso Atualizado", "content": "Conteúdo Atualizado", "is_active": False}
    response = client.put(
        f"/notices/{notice_id}?lodge_id=1", json=update_data, headers={"Authorization": f"Bearer {webmaster_token}"}
    )
    assert response.status_code == 200
    assert response.json()["title"] == "Aviso Atualizado"

    # 4. Delete Notice
    response = client.delete(f"/notices/{notice_id}?lodge_id=1", headers={"Authorization": f"Bearer {webmaster_token}"})
