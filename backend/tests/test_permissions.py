...
def test_create_permission_success(client, super_admin_token):
    ...
    response = client.post(
        "/permissions/",
        json={"action": "members:create", "description": "Permite criar membros", "min_credential": 50},
...
def test_create_permission_duplicate(client, super_admin_token, sample_permission):
    ...
    response = client.post(
        "/permissions/",
        json={"action": sample_permission.action, "description": "Outra descrição", "min_credential": 100},
        headers={"Authorization": f"Bearer {super_admin_token}"},
    )
...
def test_read_permissions(client, super_admin_token, sample_permission):
    ...
    response = client.get("/permissions/", headers={"Authorization": f"Bearer {super_admin_token}"})
...
def test_read_permission_details(client, super_admin_token, sample_permission):
    ...
    response = client.get(
        f"/permissions/{sample_permission.id}", headers={"Authorization": f"Bearer {super_admin_token}"}
    )
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
