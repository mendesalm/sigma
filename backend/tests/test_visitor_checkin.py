...
def test_perform_visitor_check_in_success(db_session, active_session):
    ...
    attendance = session_service.perform_visitor_check_in(db_session, active_session.id, visitor.id, user_lat, user_lon)
...
def test_perform_visitor_check_in_geofence_fail(db_session, active_session):
    ...
    with pytest.raises(HTTPException) as excinfo:
        session_service.perform_visitor_check_in(db_session, active_session.id, visitor.id, user_lat, user_lon)
...
def test_api_nearest_active_session(client, db_session, active_session):
    ...
    response = client.get("/masonic-sessions/nearest-active", params={"latitude": -15.7943, "longitude": -47.8823})
...
def test_api_visitor_check_in(client, db_session, active_session):
    ...
    with patch("services.session_service.perform_visitor_check_in") as mock_perform:
        ...
        response = client.post(
            ...
            json={"visitor_id": visitor_id, "latitude": -15.7943, "longitude": -47.8823},
        )
