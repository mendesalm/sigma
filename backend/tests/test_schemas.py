...
class TestMemberSchema:
    ...
    def test_future_birth_date_rejected(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            MemberCreate(full_name="João Silva", email="joao@test.com", password="TestPassword123", birth_date=tomorrow)
    ...
    def test_underage_rejected(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            MemberCreate(
                full_name="João Silva", email="joao@test.com", password="TestPassword123", birth_date=ten_years_ago
            )
...
class TestSessionSchema:
    ...
    def test_old_date_rejected(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            MasonicSessionCreate(title="Sessão Teste", session_date=old_date, status="AGENDADA")
