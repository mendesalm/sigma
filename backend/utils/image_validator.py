...
async def validate_image(
    ...
    if file.content_type not in allowed_types:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Tipo de arquivo não permitido. Use: JPEG, PNG, GIF ou WebP"
        )
    ...
    if file_size == 0:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Arquivo está vazio")
...
async def validate_image_light(file: UploadFile) -> bytes:
    ...
    if len(contents) > MAX_FILE_SIZE:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Arquivo muito grande. Máximo permitido: {max_mb:.0f}MB"
        )
...
async def resize_image_if_needed(
    contents: bytes, max_width: int = 800, max_height: int = 800, quality: int = 85
) -> bytes | None:
    """
