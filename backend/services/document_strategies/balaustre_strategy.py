...
class BalaustreStrategy(DocumentStrategy):
    ...
    async def collect_data(self, db: Session, session_id: int, **kwargs) -> dict:
        ...
        session_date_full = f"{session.session_date.day} de {months_pt[session.session_date.month]} de {session.session_date.year}"
        ...
        direct_keys = [
            ...
            "SacoProposta", "OrdemDia", "TempoInstrucao", "Tronco", "Palavra",
            "SessaoAnterior", "DataAssinatura", "Encerramento", "ValorTronco",
            "Hospitaleiro", "Tesoureiro", "Chanceler", "PrimeiroVigilante", "SegundoVigilante"
        ]
    ...
    def get_preview_context(self, db: Session, lodge_id: int | None, settings: dict, session_id: int | None = None) -> dict:
        """
