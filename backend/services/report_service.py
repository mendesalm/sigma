...
class ReportService:
    ...
    async def generate_members_report_pdf(
        db: Session, lodge_id: int, show_email: bool = False, show_phone: bool = False
    ) -> bytes:
        # 1. Buscar a Loja
        ...
        for mem in members_db:
            ...
            active_role_history = next(
                (
                    h
                    for h in mem.role_history
                    if h.lodge_id == lodge_id and h.start_date <= today and (h.end_date is None or h.end_date >= today)
                ),
                None,
            )
