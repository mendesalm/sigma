...
from models.models import LibraryItem, ItemStatusEnum
...
class LibraryItemService:
    ...
    @staticmethod
    def get_item(db: Session, item_id: int, lodge_id: int) -> Optional[LibraryItem]:
        return db.query(LibraryItem).filter(
            and_(LibraryItem.id == item_id, LibraryItem.lodge_id == lodge_id)
        ).first()

    @staticmethod
    def list_items(db: Session, lodge_id: int, book_id: Optional[int] = None, skip: int = 0, limit: int = 100) -> List[LibraryItem]:
    ...
    @staticmethod
    def update_item(db: Session, item_id: int, lodge_id: int, item_in: LibraryItemUpdate) -> Optional[LibraryItem]:
