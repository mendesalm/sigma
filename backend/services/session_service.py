...
async def generate_session_document(
    ...
    """
    get_session_by_id(db, session_id, current_user_payload)  # Garante acesso à sessão
    ...
    if document_type.upper() == "BALAUSTRE":
        ...
        if not extra_data or "member_id" not in extra_data:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="ID do membro é obrigatório para gerar certificado."
            )
        background_tasks.add_task(
            doc_gen_service.generate_certificate_pdf_task, session_id, extra_data["member_id"], current_user_payload
        )
        ...
        background_tasks.add_task(
            ...
            doc_type_key="balaustre_eleitoral",
        )
...
async def get_balaustre_draft(db: Session, session_id: int, current_user_payload: dict) -> dict:
    """
...
async def generate_custom_session_document(
    ...
    if document_type.upper() == "BALAUSTRE":
        ...
        background_tasks.add_task(
            doc_gen_service.generate_balaustre_pdf_task, session_id, current_user_payload, custom_content=custom_content
        )
        return {"message": "Geração do Balaústre personalizado iniciada."}
    else:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Tipo de documento inválido para personalização."
        )
...
async def generate_signed_session_document(
    ...
    if document_type.upper() == "BALAUSTRE":
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Tipo de documento inválido para assinatura."
        )


async def preview_balaustre(db: Session, session_id: int, custom_content: dict, current_user_payload: dict) -> bytes:
    """
...
async def regenerate_balaustre_text(db: Session, session_id: int, custom_data: dict, current_user_payload: dict) -> str:
    """
...
def get_session_attendance(db: Session, session_id: int, current_user_payload: dict) -> list[models.SessionAttendance]:
    """
...
def update_manual_attendance(
    ...
    if user_type == "member" and active_role != "Chanceler":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Apenas o Chanceler pode atualizar manualmente a presença."
        )
    ...
    attendance = (
        db.query(models.SessionAttendance)
        .filter(models.SessionAttendance.session_id == session_id, models.SessionAttendance.member_id == member_id)
        .first()
    )
    ...
    if not attendance:
        ...
        attendance = models.SessionAttendance(
            session_id=session_id, member_id=member_id, attendance_status=status, check_in_method="MANUAL"
        )
        db.add(attendance)
    else:
...
def register_visitor_attendance(
    ...
    if user_type == "member" and active_role != "Chanceler":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Apenas o Chanceler pode registrar visitantes."
        )
    ...
    attendance = (
        db.query(models.SessionAttendance)
        .filter(models.SessionAttendance.session_id == session_id, models.SessionAttendance.visitor_id == visitor.id)
        .first()
    )

    if not attendance:
        attendance = models.SessionAttendance(
            session_id=session_id, visitor_id=visitor.id, attendance_status="Presente", check_in_method="MANUAL"
        )
        db.add(attendance)
        db.commit()
        db.refresh(attendance)
...
def perform_check_in(
    ...
    if lodge.qr_code_id != qr_code_token:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="QR Code inválido para esta Loja.")
    ...
    if not geo_service.is_within_radius(latitude, longitude, lodge.latitude, lodge.longitude, radius):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Você está fora do raio permitido para check-in nesta Loja."
        )
...
def find_nearest_active_session(db: Session, latitude: float, longitude: float) -> models.MasonicSession | None:
    """
    ...
    active_sessions = (
        ...
        .options(joinedload(models.MasonicSession.lodge))
        .all()
    )
...
def perform_visitor_check_in(
    ...
    if db_session.status != "EM_ANDAMENTO":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Check-in não permitido. A sessão não está em andamento."
        )
