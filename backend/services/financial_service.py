...
def update_lodge_financial_config(db: Session, lodge_id: int, config_update: schemas.LodgeFinancialConfigCreate) -> models.LodgeFinancialConfig:
...
# --- Categories ---
def get_categories(db: Session, lodge_id: int, type: Optional[schemas.TransactionType] = None) -> List[models.FinancialCategory]:
...
def update_category(db: Session, lodge_id: int, category_id: int, category_update: schemas.FinancialCategoryBase) -> Optional[models.FinancialCategory]:
    db_category = db.query(models.FinancialCategory).filter(
        models.FinancialCategory.id == category_id,
        models.FinancialCategory.lodge_id == lodge_id
    ).first()
    if not db_category:
        return None
...
def get_financial_transaction_by_id(db: Session, transaction_id: int, current_user_payload: dict) -> Optional[models.FinancialTransaction]:
    lodge_id = current_user_payload.get("lodge_id")
...
def create_financial_transaction(db: Session, transaction_data: schemas.FinancialTransactionCreate, current_user_payload: dict) -> models.FinancialTransaction:
    lodge_id = current_user_payload.get("lodge_id")
...
def update_financial_transaction(db: Session, transaction_id: int, transaction_update: schemas.FinancialTransactionUpdate, current_user_payload: dict) -> Optional[models.FinancialTransaction]:
    lodge_id = current_user_payload.get("lodge_id")
