...
// Components
import VariablePalette from '@/modules/documents/components/DocumentBuilder/VariablePalette';
import TiptapEditor, { TiptapEditorRef } from '@/modules/documents/components/DocumentBuilder/TiptapEditor';
import ElementSelector from '@/modules/documents/components/DocumentBuilder/ElementSelector';
...
const DocumentBuilder: React.FC<DocumentBuilderProps> = ({ mode = 'lodge', lodgeId: propLodgeId }) => {
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>, section: string, field: string) => {
        ...
        try {
            const uploadUrl = mode === 'admin' 
                ? `/admin/templates/upload_asset` 
                : `/lodges/${effectiveLodgeId}/upload_asset`;
    ...
    const buildPayloadV2 = (settings: typeof allSettings) => {
        ...
        for (const [docType, docSettings] of Object.entries(settings)) {
            ...
            const page_settings = {
                ...
                page_padding: s.page_padding || '0cm',
                ...
                show_page_numbers: s.show_page_numbers ?? true
            };
            ...
            {activeStep === 1 && (
                ...
                <AccordionDetails>
                    <Grid container spacing={2}>
                        ...
                        <Grid size={{ xs: 6 }}>
                            <TextField label="Margens Externas" size="small" fullWidth value={currentSettings.styles.page_margin || '1cm'} onChange={(e) => updateCurrentSetting('page_margin', e.target.value, true)} />
                        </Grid>
                        <Grid size={{ xs: 6 }}>
                            <TextField label="Espaçamento Interno (Padding)" size="small" fullWidth value={currentSettings.styles.page_padding || '0cm'} onChange={(e) => updateCurrentSetting('page_padding', e.target.value, true)} />
                        </Grid>
                        <Grid
                            size={{
                                xs: 6
