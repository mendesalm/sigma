...
Uma refatoração completa da estilização do frontend foi realizada para garantir consistência visual, adesão ao tema escuro e aproveitamento total da largura da página, além de aprimorar a experiência de navegação e a estética dos formulários.
...
*   **Layout Adaptativo e Responsivo:**
    *   **`PublicLayout.tsx`:** O container principal de rotas públicas foi configurado para atuar como um *scroll snap container*, com `scroll-snap-type: y mandatory`, garantindo uma navegação suave e seccionada entre os componentes filhos.
    ...
    *   **`Header.tsx`:** O componente de cabeçalho foi refatorado para utilizar `AppBar`, `Toolbar` e `Typography` do Material-UI. Sua cor de fundo foi padronizada para preto (`#000000`). O logo Sigma recebeu um `filter: drop-shadow()` para maior destaque. Adicionalmente, o link de "Login" foi substituído condicionalmente por um link "Home" quando o usuário está na página de login, oferecendo uma navegação intuitiva de retorno à landing page.
    *   **`Footer.tsx`:** O componente de rodapé foi simplificado, utilizando `Box` e `Typography` do Material-UI, e sua integração ao layout foi ajustada. O texto de atribuição "Desenvolvido por [Seu Nome/Empresa]" foi alterado para "Desenvolvido por Dantec". O rodapé também foi configurado com `scroll-snap-align: start` para integrar-se ao sistema de *scroll snapping*.
...
*   **Páginas Principais Refatoradas:**
    *   **`LandingPage.tsx`:** A página inicial foi completamente reescrita com componentes do Material-UI (`Box`, `Container`, `Typography`, `Grid`, `Paper`). Garantindo um design full-width, responsivo e aderente ao tema escuro. Adição do logotipo Sigma centralizado acima dos títulos na seção hero, com um `filter: drop-shadow()`. Um efeito de *blur* (`backdrop-filter`) foi aplicado abaixo do header para aprimorar a separação visual. O texto teve sua cor ajustada e revertida para `text.primary` do tema. A página foi integrada ao sistema de *scroll snapping*, com o hero e a seção de *features* (agora agrupada para o snap) como pontos de alinhamento.
    *   **`LoginPage.tsx`:** A página de login foi refatorada para usar componentes do Material-UI (`Container`, `Box`, `Typography`, `TextField`, `Button`, `Link`, `CircularProgress`, `FormControlLabel`, `Checkbox`, `Alert`). Recebeu a mesma estilização de background e o efeito de *blur* da `LandingPage`. O `logoSigma` foi aplicado acima do formulário. O formulário de login foi aprimorado com um efeito *glassmorphism* (fundo semi-transparente, `backdrop-filter: blur()`, borda sutil e sombra refinada), e sua transparência foi aumentada. A largura do formulário foi ajustada para `600px`. Os links "Esqueci a senha" e "Não tem uma conta? Solicitar cadastro" foram organizados verticalmente. O texto teve sua cor ajustada e revertida para `text.primary` do tema.
